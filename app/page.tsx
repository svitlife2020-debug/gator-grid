...
import {
  ...
  Globe, ExternalLink, BookOpen, AlertTriangle,
} from "lucide-react";
...
function inputCls(err?: boolean) {
  return ["w-full rounded-xl px-4 py-3.5 text-base font-medium transition-all duration-200 outline-none border focus:ring-2",
    err ? "border-red-400 bg-red-50 focus:ring-red-100" : "bg-white focus:bg-white"].join(" ");
}
...
function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full transition-all duration-500"
      style={{ background: scrolled ? "rgba(255,255,255,0.98)" : "white", borderBottom:`1px solid ${C.border}44`,
        boxShadow: scrolled ? "0 4px 32px rgba(45,155,78,0.08)" : "none", backdropFilter: scrolled ? "blur(12px)" : "none" }}>
        ...
        <div className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map(l => (
            <a key={l.label} href={l.href}
              className="px-3 py-2 rounded-lg text-sm font-bold tracking-wider uppercase transition-all hover:bg-green-50"
          ...
          <a href="https://bceptafl.givebacks.com/shop" target="_blank" rel="noopener noreferrer"
            className="ml-3 px-5 py-2.5 rounded-full text-sm font-black tracking-wider uppercase hover:opacity-90"
            style={{ background:C.green, color:"white", letterSpacing:"0.08em" }}>
            PTA Shop
          </a>
        </div>
        <div className="md:hidden relative" ref={menuRef}>
          <button onClick={() => setOpen(v => !v)} className="p-2 rounded-lg hover:bg-green-50" aria-label="Menu">
            ...
            <div className="absolute right-0 top-14 w-56 rounded-2xl overflow-hidden z-50"
              ...
              {NAV_LINKS.map(l => (
                <a key={l.label} href={l.href} onClick={() => setOpen(false)}
                  className="flex items-center px-5 py-3.5 text-sm font-bold uppercase tracking-wider hover:bg-green-50"
              ...
              <a href="https://bceptafl.givebacks.com/shop" target="_blank" rel="noopener noreferrer"
                ...
                className="flex items-center px-5 py-3.5 text-sm font-bold uppercase tracking-wider"
    ...
    <section id="top" className="relative overflow-hidden" style={{ background:C.green, minHeight:600 }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-24 flex flex-col items-center text-center">
        ...
        <div className="relative mb-9">
          ...
          <Image src={LOGO_URL} alt="Bessey Creek Elementary" width={160} height={160} priority
        ...
        <h1 className="mb-6" style={{ fontFamily:"'Playfair Display',Georgia,serif" }}>
          <span className="block text-xl font-bold tracking-[0.25em] uppercase mb-2" style={{ color:C.gold }}>Welcome to</span>
          <span className="block font-black" style={{ color:"white", fontSize:"clamp(3.2rem,7vw,6rem)", lineHeight:1.02, letterSpacing:"-0.025em" }}>The Gator Grid</span>
        </h1>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4">
          <a href="#volunteer" className="px-10 py-4 rounded-full text-base font-black tracking-wider uppercase transition-all duration-300 hover:scale-[1.03]"
...
function DashboardSection({ data }: { data: DashboardData | null }) {
  ...
  const d: DashboardData = {
    ...
    nextEventName: raw.nextEventName, nextEventDate: raw.nextEventDate,
  };
...
// EVENTS
// ═════════════════════════════════════════════════════════════
const CAT_COLORS: Record<string,string> = { Community:C.green, Meeting:C.blue, Academic:"#7c3aed", Fundraiser:C.gold, Spirit:C.red };
  ...
  return (
    <Section id="volunteer" cream>
      ...
      <div className="max-w-2xl mx-auto">
        <div className="rounded-3xl overflow-hidden relative" style={{ background:"white", border:`1px solid ${C.border}`, boxShadow:"0 20px 70px rgba(45,155,78,0.12)" }}>
          ...
          <div className="p-10 space-y-6">
            ...
            {status==="error" && <div className="flex items-center gap-3 p-4 rounded-xl" style={{ background:"#fef2f2", border:"1px solid #fecaca", color:C.red }}><AlertCircle className="w-5 h-5 flex-shrink-0" /><span className="text-base font-semibold">{errorMsg}</span></div>}
            <button onClick={handleSubmit} disabled={status==="loading"}
              className={["w-full py-4 rounded-xl text-base font-black uppercase transition-all duration-300 flex items-center justify-center gap-2", status==="loading"?"opacity-50 cursor-not-allowed":"hover:opacity-90 hover:shadow-xl hover:-translate-y-0.5"].join(" ")}
              ...
              {status==="loading" ? <><Loader2 className="w-5 h-5 animate-spin" />Submitting…</> : <><Sparkles className="w-5 h-5" />Submit Volunteer Hours</>}
            </button>
          </div>
        </div>
      </div>
...
function WishlistSection({ items }: { items: WishlistItem[] | null }) {
  ...
  return (
    <Section id="wishlist" cream>
      ...
      <div className="flex flex-wrap justify-center gap-3 mb-10">
        {(["All","High","Medium","Low"] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={{
              background: filter===f ? C.green : "white",
              color:       filter===f ? "white" : C.muted,
              border:      `1px solid ${filter===f ? C.green : C.border}`,
              boxShadow:   filter===f ? `0 4px 16px ${C.green}44` : "none",
            }}>
            {f === "All" ? "All Items" : f}
          </button>
        ))}
          ...
          return (
            <div key={item.id ?? i} className="rounded-2xl p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1"
              ...
              <div className="flex items-start justify-between gap-3 mb-3">
                ...
                <span className="text-xs font-black px-2.5 py-1 rounded-full" style={{ background:p.bg, color:p.color }}>
                  {p.label}
                </span>
              </div>
              ...
              {item.storeLink ? (
                <a href={item.storeLink} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-black uppercase tracking-wider transition-all hover:opacity-90"
      ...
      {filtered.length === 0 && (
        <p className="text-center py-10 text-base font-semibold" style={{ color:C.muted }}>
          No items at this priority level right now. 🎉
        </p>
      )}

      <p className="text-center text-sm font-semibold mt-8" style={{ color:C.muted }}>
        Teachers: update your wishlist in the school&apos;s Google Sheet · Questions?{" "}
...
function SponsorCard({ sponsor }: { sponsor:Sponsor }) {
  ...
  return (
    <div className="rounded-2xl p-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1"
      style={{ background:isGold ? "#f5edd6" : "white", border:`1px solid ${isGold ? C.gold+"88" : C.border}`, boxShadow:isGold ? `0 6px 30px ${C.gold}44` : "0 4px 16px rgba(0,0,0,0.05)" }}>
      <div className="flex items-start gap-4">
        <div className="w-16 h-16 rounded-xl flex items-center justify-center flex-shrink-0 font-black text-2xl"
          ...
          {sponsor.logoUrl
            ? <Image src={sponsor.logoUrl} alt={sponsor.name} width={64} height={64} className="object-contain rounded-xl" />
            : sponsor.name[0]}
        </div>
        <div className="flex-1 min-w-0">
          ...
          {sponsor.website && (
            <a href={sponsor.website} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-sm font-black px-3 py-1.5 rounded-lg transition-all hover:opacity-80"
  ...
  return (
    <Section id="sponsors">
      ...
      <p className="text-center text-base font-semibold" style={{ color:C.muted }}>
        ...
        <a href="#contact" className="font-black underline underline-offset-2" style={{ color:C.green }}>Contact the PTA →</a>
      </p>
    </Section>
  );
}
        ...
        <div className="md:col-span-3">
          <div className="rounded-3xl overflow-hidden relative" style={{ background:"white", border:`1px solid ${C.border}`, boxShadow:"0 20px 70px rgba(45,155,78,0.12)" }}>
            ...
            {status==="success" && (
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-6 rounded-3xl"
                ...
                <div className="text-center">
                  ...
                  <p className="text-base font-semibold" style={{ color:C.muted }}>We&apos;ll get back to you soon. A confirmation email is on its way.</p>
                </div>
            ...
            <div className="p-10 space-y-6">
              ...
              {status==="error" && <div className="flex items-center gap-3 p-4 rounded-xl" style={{ background:"#fef2f2", border:"1px solid #fecaca", color:C.red }}><AlertCircle className="w-5 h-5 flex-shrink-0" /><span className="font-semibold">{errorMsg}</span></div>}
              <button onClick={handleSubmit} disabled={status==="loading"}
                className={["w-full py-4 rounded-xl text-base font-black uppercase transition-all duration-300 flex items-center justify-center gap-2", status==="loading"?"opacity-50 cursor-not-allowed":"hover:opacity-90 hover:shadow-xl hover:-translate-y-0.5"].join(" ")}
                style={{ background: status==="loading"?"#ccc":`linear-gradient(135deg,${C.green},${C.greenDark})`, color:"white", letterSpacing:"0.12em", boxShadow: status==="loading"?"none":`0 6px 32px ${C.green}66` }}>
...
## 📊 PART 4 — Budget Tab formula (for the Treasurer)

In your **Budget** sheet, put this in cell **F3** and drag down:
```
=IF(E3="","",F2+IF(D3="Income",E3,-E3))
```

And in **F2** (your opening balance row), just type your starting number, e.g. `5000`.

To get a **summary dashboard** in the Budget tab, add these somewhere (e.g. H1:I4):
```
H1: Total Income       I1: =SUMIF(D:D,"Income",E:E)
H2: Total Expenses     I2: =SUMIF(D:D,"Expense",E:E)
H3: Net Balance        I3: =I1-I2
H4: Opening Balance    I4: (type your starting balance here manually)
